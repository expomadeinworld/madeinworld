...
# Data source for database password from Secrets Manager
...
resource "aws_ecr_repository" "catalog_service" {
  ...
  tags = {
    Name        = "${var.project_name}-${var.environment}-catalog-service-ecr"
    Environment = var.environment
    Project     = var.project_name
  }
}
...
resource "aws_ecr_repository_policy" "catalog_service" {
